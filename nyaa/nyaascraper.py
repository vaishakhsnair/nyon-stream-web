...
baseUrl: str = 'https://nyaa.iss.one/?s=seeders&o=desc'
webtorrentArgs: str = "--keep-seeding --mpv --playlist -o ../tmp"
maxPageNum: int = 2
...
#FOR WINDOWS ONLY
webtorrent_path = os.path.join("dependencies","webtorrent-cli","bin","cmd.js") #Webtorrent node module
node_path = os.path.join("dependencies","node.exe") #Node Executable
mpv_path = os.path.join("dependencies") #Folder containing mpv
...
def getTorrents(query: str) -> dict:
    url = f"{baseUrl}&q={query}"
    logging.info(f"Got Query :{url}")
    torrents = []
